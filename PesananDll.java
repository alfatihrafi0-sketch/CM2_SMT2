public class PesananDll {
    ...
    void laporanPesanan() {
        ...
        // if
        // (current.data.namaPesanan.compareToIgnoreCase(current.next.data.namaPesanan)
        // > 0) {
        ...
        // Modifikasi Menjadi insertion sort
        NodePesanan i = head.next;

        // Perulangan dimulai dari node kedua
        while (i != null) {

            // Menyimpan data yang akan dibandingkan
            Pesanan key = i.data;

            NodePesanan j = i.prev; // Pointer j digunakan untuk bergerak ke kiri

            // Selama data sebelumnya lebih besar dari key
            while (j != null &&
                    j.data.namaPesanan.compareToIgnoreCase(key.namaPesanan) > 0) {

                j.next.data = j.data; // Menggeser data sebelumnya ke kanan

                j = j.prev; // Bergerak ke node sebelumnya
            }
            if (j == null) { // jika j menjadi null berarti data ditempatkan di head
